...
// Get ALL items for global cache (used for tooltips everywhere)
...
app.get('/api/market-rates', async (req, res) => {
    try {
        ...
        // Return only boss drops - converted items appear in "Converts To" column via itemDataCache lookups
        res.json(dropResult.rows);
    } catch (error) {
...
app.get('/api/points-confirmation', async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `SELECT points_system_confirmed_at FROM users WHERE id = $1`,
            [userId]
        );

        if (result.rows.length === 0 || !result.rows[0].points_system_confirmed_at) {
            res.json({ confirmed: false });
        } else {
            res.json({
                confirmed: true,
                confirmed_at: result.rows[0].points_system_confirmed_at
            });
        }
    } catch (error) {
...
app.post('/api/points-confirmation', async (req, res) => {
    try {
        ...
        // Update user's confirmation timestamp
        const result = await pool.query(
            `UPDATE users
             SET points_system_confirmed_at = NOW()
             WHERE id = $1
             RETURNING points_system_confirmed_at`,
            [user_id]
        );
